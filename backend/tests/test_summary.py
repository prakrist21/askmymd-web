...
from app.db import documents, get_document, update_document, Document, get_chunks
...
def test_ask_uses_cached_summary_no_groq_call_on_cold_start():
    ...
    # Create doc with content via DB proxy (bypassing POST to control summary directly)
    # Use documents proxy to insert, then set summary via update_document
    ...
    # Ensure summary is stored
    stored = get_document(doc.id)
    assert stored is not None
    # If proxy didn't store summary, update explicitly
    ...
    # Insert chunks for retrieval (so ask has something to search)
    from app.db import insert_chunks
    from app.services.rag_service import chunk_markdown

    chunks = chunk_markdown(stored.content)
    # Ensure chunks exist (if not already, insert)
    if not get_chunks(doc.id):
        insert_chunks(doc.id, chunks)

    # Simulate cold start: no in-memory cache to clear (DB-backed), just ensure no global state
    # Now call ask with mocks — generate_summary should never be called
    with patch("app.services.llm_service.generate_summary") as mock_summary, \
        ...
        # Critical: no Groq summary call on cold start when summary already cached
        mock_summary.assert_not_called()
        # Answer generation should have received the cached summary
...
def test_create_caches_summary_and_resync_overwrites():
    ...
    with patch("app.services.llm_service.generate_summary", return_value="summary v1 cats") as mock_gen:
        ...
        # Should have called generate_summary once on create
        assert mock_gen.call_count == 1
        stored = get_document(doc_id)
        assert stored.summary == "summary v1 cats"

    # Edit content and resync — should regenerate summary
    ...
    with patch("app.services.llm_service.generate_summary", return_value="summary v2 quantum") as mock_gen2, \
        ...
        # Also need to mock insert_chunks to avoid real embedding for this small chunk? Let it use real
